...
| **Fast Build, Install and Launch** (`launch.md`) | `lance_r`, `l_`, `l_r`, `lr_` | Compile, installe l'application sur le téléphone et lance automatiquement son activité principale (`MainActivity`). |
...
| **Fix TODOs** (`t_.md`) | `t_`, `t_models`, `t_usage`, `fix_todo` | Résout automatiquement les TODOs ou délègue aux skills correspondants (ex: `TODO: log_`, `TODO: sem_`, `TODO: con_c`), puis affiche le diff de code à la fin. |
