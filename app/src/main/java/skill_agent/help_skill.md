...
| **TODO Context Exporter** (`t_/t_contex_files_relative_pour_regle_todo.md`) | `t_contex_files_relative_pour_regle_todo`, `t_contex`, `todo_context` | Exporte le contexte d'un TODO (code environnant, fichiers relatifs, flux) dans `t_/contex.md` pour le fournir à l'IA sans tout l'historique du projet. |
| **Read and Isolate File** (`t_/read_ingor_.md`) | `read_ingor_<filename>`, `read_ignor_<filename>`, `isole_<filename>` | Sauvegarde les ignores, isole un fichier unique pour économiser des tokens, applique les correctifs, puis restaure le contexte initial. |
