package com.example.light_app_controles.B.Screens.Z.Screens.Test.ID1.Client_Map.App.Bon_Vent_Etate.View.ID1.FeatureID1_BigDataBase_Editeur_Par_Csv_Floating_Separated_Button.Feature.Options.M14.Actions
...
fun M14_FragMap_DropdownMenu(
    ...
    LaunchedEffect(Unit) {
        runCatching {
            val counts = vm.setter_LongDatas.get_Firebase_M14_Counts(
                M14VentPeriode.ref_Test
            )
    ...
    LaunchedEffect(vm.active_Datas.list_M14Period, csvRefreshTrigger) {
        withContext(Dispatchers.IO) {
            val csv = M14VentPeriode.csv_test
            if (csv.exists() && csv.length() > 0L) {
                ...
                if (lines.size >= 2) {
                    ...
                    val roomKeys = vm.active_Datas.list_M14Period
    ...
    if (showExportRoomToCsvConfirm) {
        AvertissementDialog(
            ...
            message      = "سيتم تصدير فترات المبيعات Room إلى ملف CSV.\nهل تريد المتابعة؟",
            confirmLabel = "تصدير",
            onConfirm    = {
                showExportRoomToCsvConfirm = false
                coroutineScope.launch(Dispatchers.IO) {
                    vm.setter_LongDatas.export_M14_Room_To_Csv(
                        csv = M14VentPeriode.csv_test,
                    )
    ...
    if (showExportCsvToFirebaseConfirm) {
        AvertissementDialog(
            ...
            message = "سيتم رفع بيانات فترات المبيعات من ملف CSV إلى Firebase.\nهل تريد المتابعة؟",
            confirmLabel = "رفع",
            onConfirm = {
                showExportCsvToFirebaseConfirm = false
                coroutineScope.launch(Dispatchers.IO) {
                    runCatching {
                        vm.setter_LongDatas.set_scv_M14_au_fireBase(
    ...
    if (showImportCsvToRoomConfirm) {
        AvertissementDialog(
            ...
            message      = "سيتم استيراد فترات المبيعات من ملف CSV وتحديث قاعدة البيانات المحلية.\nهل تريد المتابعة؟",
    ...
    if (showImportFirebaseToCsvConfirm) {
        AvertissementDialog(
            ...
            message      = "سيتم تحميل فترات المبيعات من Firebase وحفظها في ملف CSV.\nهل تريد المتابعة؟",
            confirmLabel = "استيراد",
            onConfirm    = {
    ...
    if (showDeleteAllRoomConfirm) {
        AvertissementDialog(
            ...
            message      = "سيتم حذف جميع فترات المبيعات من قاعدة البيانات المحلية بشكل نهائي.\nهل تريد المتابعة؟",
            confirmLabel = "حذف",
            onConfirm    = {
                showDeleteAllRoomConfirm = false
                coroutineScope.launch(Dispatchers.IO) {
                    vm.setter_LongDatas.delete_All_M14()
                    vm.reload()
                    withContext(Dispatchers.Main) { onDismiss() }
                }
            },
    ...
    if (showImportFirebaseToRoomConfirm) {
        AvertissementDialog(
            ...
            message      = "سيتم تحميل فترات المبيعات مباشرة من Firebase وحفظها في قاعدة البيانات المحلية.\nهل تريد المتابعة؟",
            confirmLabel = "استيراد",
            onConfirm    = {
                showImportFirebaseToRoomConfirm = false
                coroutineScope.launch(Dispatchers.IO) {
                    runCatching {
                        vm.setter_LongDatas.import_M14_FireBase_To_Room(
                            refDataBase = M14VentPeriode.ref_Test,
                        )
                        vm.reload()
                    }
    ...
    DropdownMenu(
        ...
        HorizontalDivider(thickness = 3.dp, color = Color(0xFF3F51B5))
        HorizontalDivider()
        Text("FireBase")
        DropdownMenuItem(
            leadingIcon = {
